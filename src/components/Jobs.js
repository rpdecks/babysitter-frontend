...
class Jobs extends React.Component {
  ...
  interestBtn = (job_id) => {
    if (
      this.props.interestedJobs &&
      this.props.interestedJobs.find((job) => job.job_id === job_id)
    ) {
      return (
        <Button
          size="sm"
  ...
  applyCompletionFilter = (job) => {
    ...
    else if (this.props.completionFilter === "complete")
      return job.status === "complete";
    else if (this.props.completionFilter === "incomplete")
  ...
  applyFirstAidCertFilter = (job) => {
    if (this.props.firstAidCertFilter === true)
      return job.first_aid_cert === true;
    else return true;
  };
  ...
  mapMyJobs = (jobs) => {
    let jobsCopy = [...jobs];
    const myFilters = [
      this.applyCompletionFilter,
      this.applyNonSmokingFilter,
      this.applyPetsFilter,
      this.applyFirstAidCertFilter,
    ];
    const filteredJobs = jobsCopy.filter((job) =>
      myFilters.every((f) => f(job))
    );
    return filteredJobs.map((job, index) => {
      return (
        <tr key={index}>
          <td>
            <Link to={`/jobs/${job.id}`}>Show</Link>
          </td>
        ...
        <Table striped bordered hover size="sm">
          <thead>
            <tr>
              ...
              <th onClick={() => this.props.sortTable("total_child_count")}>
                Kids
              </th>
              <th>Non-Smoking?</th>
              <th>First-aid</th>
          ...
          <tbody>{jobs ? this.mapMyJobs(jobs) : null}</tbody>
        </Table>
      </>
    );
  };
  ...
  renderHeader = () => {
    if (window.location.pathname === "/past-jobs") {
      return <div className="header-text">Past Babysitting Jobs</div>;
    } else if (window.location.pathname === "/pending-jobs") {
      return <div className="header-text">Pending Babysitting Jobs</div>;
    } else if (
      window.location.pathname === "/jobs" &&
      this.props.userType === "caregiver"
    ) {
      return <div className="header-text">Available Babysitting Jobs</div>;
    }
  };

  renderJobs = () => {
    if (window.location.pathname === "/past-jobs") {
      let filteredJobs = this.props.userJobs.filter(
          ...
          <Table striped bordered hover size="sm">
            <div className="table">
              <thead>
                <tr>
                  <th>Details</th>
                  <th onClick={() => this.props.sortTable("start_time")}>
                    Date
                  </th>
                  <th onClick={() => this.props.sortTable("start_time")}>
                    Start
                  </th>
                  <th onClick={() => this.props.sortTable("length")}>
                    Duration
                  </th>
                  <th>Title</th>
                  <th>Location</th>
                  <th onClick={() => this.props.sortTable("pay_rate")}>
                    Pay rate
                  </th>
                  <th onClick={() => this.props.sortTable("total_child_count")}>
                    Kids
              ...
              <div className="table">
                <thead>
                  <tr>
                    ...
                    <th
                      onClick={() => this.props.sortTable("total_child_count")}
                    >
                      Kids
                    </th>
                    <th>Non-Smoking?</th>
                    <th>First-aid</th>
                    <th>Pets?</th>
                    <th onClick={() => this.props.sortTable("status")}>
                      Status
                    </th>
                  </tr>
                </thead>
...
const mapDispatchToProps = (dispatch) => {
  return {
    addInterested: (jobId) =>
      dispatch({ type: "ADD_CANDIDATE", payload: jobId }),
    removeInterested: (jobId) =>
      dispatch({ type: "REMOVE_CANDIDATE", payload: jobId }),
    sortTable: (criteria) => dispatch({ type: "SORT_BY", sortBy: criteria }),
    switchOrder: (criteria) =>
      dispatch({ type: "SWITCH_ORDER", ascending: criteria }),
    setSelectedJob: (job) =>
      dispatch({ type: "SET_SELECTED_JOB", selectedJob: job }),
  };
};
